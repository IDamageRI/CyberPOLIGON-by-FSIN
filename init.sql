...
-- представление списка турниров на которые подали заявки
...
GROUP BY v.vacancy_id, v.title -- группировка по турнирам
HAVING COUNT(a.application_id) > 0 -- только те, где есть заявки
ORDER BY application_count DESC;



-- представление списка турниров, на которые не было подано заявок
...
WHERE a.application_id IS NULL -- турниры без заявок
ORDER BY v.title;
